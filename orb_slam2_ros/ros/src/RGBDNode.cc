...
RGBDNode::~RGBDNode () {
  ...
  std::string trajectory_path, sequence_number;
  node_handle_.getParam(name_of_node_ + "/trajectory_path", trajectory_path);
  node_handle_.getParam(name_of_node_ + "/sequence_number", sequence_number);
  
  trajectory_path += std::string("kitti_")+sequence_number+std::string("_odo_result.txt");

  orb_slam_->Shutdown();
  orb_slam_->SaveTrajectoryKITTI(trajectory_path);
  // Save camera trajectory

  std::cout<<"finish save path in viual lidar mode "<<std::endl;
