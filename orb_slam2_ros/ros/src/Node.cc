...
void Node::LoadOrbParameters (ORB_SLAM2::ORBParameters& parameters) {
  ...
  node_handle_.param(name_of_node_ + "/load_calibration_from_cam", load_calibration_from_cam, false);
        std::cout<<"load_calibration_from_cam:::"<<load_calibration_from_cam<<std::endl;
  ...
  if (load_calibration_from_cam) {
    ...
    node_handle_.getParam(name_of_node_ + "/dataset_folder", dataset_folder);
    node_handle_.getParam(name_of_node_ + "/sequence_number", sequence_number);
    ...
    std::cout<<"calib_path"<<calib_path<<std::endl;
    ...
    float b_rgb = (T_cam3_velo.inverse()*p_cam - T_cam2_velo.inverse()*p_cam).norm()*1000;   


    parameters.fx = calibs[2][0];
    parameters.fy = calibs[2][5];
    parameters.cx = calibs[2][2];
    parameters.cy = calibs[2][6];

    parameters.baseline =b_rgb;
